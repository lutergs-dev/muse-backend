...
import dev.lutergs.muse.infra.rest.muse.dto.UserChangeNickNameDto
import dev.lutergs.muse.infra.rest.muse.dto.UserLoginDto
...
class UserInfoRestHandler(
  ...
  fun getUser(request: ServerRequest): ServerResponse {
    return this.validateAccessToken(request) { _ ->
      val id = runCatching { request.pathVariable("id").toLong() }
        .getOrNull()
      if (id != null) {
  ...
  fun getUsers(request: ServerRequest): ServerResponse {
    return this.validateAccessToken(request) { _ ->
      val page = request.paramOrNull("page")?.toInt()
      if (page != null) {
        this.userInfoService.getUsers(page)
          .let { ServerResponse.ok().body(it) }
      } else {
        ServerResponse.badRequest().build()
      }
  ...
  fun getUserIdFromAccessToken(request: ServerRequest): ServerResponse {
    return this.validateAccessToken(request) { userId ->
      ServerResponse.ok()
        .body(userId)
    }
